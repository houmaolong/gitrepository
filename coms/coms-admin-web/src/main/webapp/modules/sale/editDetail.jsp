...
<body>
	<div class="breadcrumbs" id="breadcrumbs">
		<ul class="breadcrumb">
			...
			<li class=""><a href="${ctxPath}/modules/sale/detail.jsp?id=${param.id}">订单详情</a></li>
			<li class="active">编辑产品</li>
		</ul>
	</div>
	<div class="page-content">
		<div class="page-header">编辑产品</div>
		<!-- /.page-header -->
		...
		<form class="form-horizontal" role="form" id="form" enctype="multipart/form-data">
			...
			<input type="hidden" id="masterId" name="masterId" value="${param.id}"/>
			...
			<div class="form-group">
				...
				<div class="col-sm-9">
					<input type="text" id="spec" placeholder="产品规格 "
						name="spec" class="col-xs-10 col-sm-5">
				</div>
			</div>
			<div class="form-group">
				<label class="col-sm-3 control-label no-padding-right"
			...
			<div class="form-group">
				<label class="col-sm-3 control-label no-padding-right"
					for="form-field-2"> 图片 </label> <label class="col-sm-9">
	...
	<script type="text/javascript">
		$("#unit").selectInit({url:'sale/unitList.ajax',value:'sName',key:"sName"});
        $(function()
        {	
            $.ajax({
                url: 'sale/getDetail.ajax',
                type: 'POST',
                dataType: 'json',
                data: {
                    id: '${param.detailId }'
                    ...
                    if(data.success == true) {
                        $("input,select,textarea").each(function()
                        {
                            ...
                            if($(this).attr("type") == "textarea") {
                                var name = $(this).attr("name");
                                $(this).html(data.data['memo']);                            
                                return;
                            }
                            if($(this).attr("name")=="deliverDate"){
                            	$(this).val(new Date(data.data[$(this).attr("name")]).format("yyyy-MM-dd"));
                            	return;
                        	}
            ...
            $("#form").validate({
                ...
                submitHandler: function()
                {
                    $(form).ajaxSubmit({
                        ...
                        url: "${ctxPath}/sale/editDetail.ajax", // 需要提交的 url
                        success: function(data)
                        {
